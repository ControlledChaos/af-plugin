...
// namespace AF_Plugin\Includes\Tools;
...
function themedrive_add_pages() {
    add_theme_page(
        ...
        __( 'Theme Test', 'af-plugin' ),
        'edit_theme_options',
        AFP_ADMIN_SLUG . '-theme-test',
...
function themedrive_switcher() {
    ...
    if ( themedrive_is_enabled() ) {
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is enabled with', 'af-plugin' ),
            $theme_name
        );
    } else {
        $get_theme  = wp_get_theme();
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is disabled', 'af-plugin' ),
            __( 'The active theme is', 'af-plugin' ),
            $theme_name
        );
    }
...
function themedrive_options_page() {
    ...
    if ( isset( $_POST['button'] ) && 'Enable Theme Drive' == $_POST['button'] ) {
        ...
        $msg_status = __( 'Live theme test has been disabled.', 'af-plugin' );
...
?>
<div class="wrap" >
    <h2><?php _e( 'Live Theme Test', 'af-plugin' ); ?></h2>
    <?php $action_url; ?>
    ...
    <?php wp_nonce_field( 'theme-drive' ); ?>
        <h2><?php _e( 'Instructions', 'af-plugin' ); ?></h2>
        <ol>
            <li><?php _e( 'Select a theme to preview live on the site from the select box below (lists all installed themes).', 'af-plugin' ); ?></li>
            <li><?php _e( 'Enable theme test', 'af-plugin' ); ?></li>
            <li><?php _e( 'Once the theme is ready to go live, disable theme test and activate the theme on the Themes page.', 'af-plugin' ); ?></li>
        </ol>
   	    <p><?php _e( 'Additionally you may add "?theme=xxx" to a URL, where xxx is the slug of the theme you want to test.', 'af-plugin' ); ?></p>
        <?php themedrive_switcher(); ?>
        <h2><?php _e( 'User Role', 'af-plugin' ); ?></h2>
        <p><?php _e( 'Specify the level of users to have access to the selected theme preview.', 'af-plugin' ); ?></p>
        <p><?php _e( 'By default it is set to 10 (admin only). Level 7 are editors, level 4 are authors and level 1 are contributors.', 'af-plugin' ); ?></p>
        <p><?php _e( 'The access level is ignored for accessing the site with ?theme=xxx paramaeter.', 'af-plugin' ); ?></p>
        <p><input style="border-width: inherit; border-style: inherit; border-color: inherit; width: 30px;" name="access_level" id="access_level" value="<?php echo esc_attr( $access_level ); ?>" /> <label for="access_level"><?php _e( 'Access level', 'af-plugin' ); ?></label></p>
        <p class="submit">
