...
if ( ! defined( 'DOING_AJAX' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
    add_action( 'admin_init', 'afp_check_wp_debug_define' );
}

function afp_check_wp_debug_define() {
	if ( defined( 'WP_DEBUG' ) && ! WP_DEBUG ) {
		$dev_mode = get_option( 'afp_site_development' );
		if ( $dev_mode ) {
			afp_set_wp_debug_define( true );
		} elseif ( false == $dev_mode )  {
			afp_set_wp_debug_define( false );
		}
	}
}
 ...
 * @return void
 */
function afp_set_wp_debug_define( $turn_it_on ) {
	...
	$config_file_path = afp_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
    }
	...
	*/
	apply_filters( 'afp_set_wp_debug_define', $turn_it_on );
 ...
 */
function afp_find_wpconfig_path() {
...
function afp_deactivation() {
    ...
    set_afp_wp_debug_off( false );
}

function set_afp_wp_debug_off( $turn_it_off ) {
	// Get path of the config file
	$config_file_path = afp_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
	 ...
	 * @param string $turn_it_on The value of WP_DEBUG constant
	*/
	apply_filters( 'afp_set_wp_debug_define', $turn_it_off );
