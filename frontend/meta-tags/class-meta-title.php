...
class Meta_Title {
	...
	public function __construct() {
		...
		add_action( 'afp_meta_title_tag', [ $this, 'title' ] );
	...
	public function title() {
		...
		$author_title = sprintf(
			'%1s %2s',
			__( 'Posts by', 'af-plugin' ),
			get_the_author_meta( 'display_name', $author_id )
		);

		// Apply a filter to author archive title.
		$author_meta = apply_filters( 'afp_author_meta_title', $author_title );

		// Custom search title.
		$search_title = sprintf(
			'%1s %2s',
			__( 'Searching', 'af-plugin' ),
			get_bloginfo( 'name' )
		);

		// Apply a filter to search title.
		$search_meta = apply_filters( 'afp_search_meta_title', $search_title );
