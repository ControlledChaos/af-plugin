...
class Meta_Description {
	...
	public function __construct() {
		...
		add_action( 'afp_meta_description_tag', [ $this, 'description' ] );
	...
	public function description() {
		...
		$search_desc      = wp_strip_all_tags(
			sprintf(
				'%1s \'%2s\'',
				__( 'Showing results for', 'af-plugin' ),
				$search_query
			)
		);

		// Apply a filter to hard-coded text.
		$search_meta_desc = apply_filters( 'afp_meta_description_search', $search_desc );
		...
		$meta_description = apply_filters( 'afp_meta_description', $description );
