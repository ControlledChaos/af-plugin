 ...
 * @link        https://controlledchaos.github.io/af-plugin/
 * @license     GPL-3.0+ http://www.gnu.org/licenses/gpl-3.0.txt
 *
 * Plugin Name:  Audrey Fisher Plugin
 * Plugin URI:   https://controlledchaos.github.io/af-plugin/
...
if ( ! class_exists( 'AF_Plugin' ) ) :
	final class AF_Plugin {
		...
		private function constants() {
			 ...
			 */
			if ( ! defined( 'AFP_DOMAIN' ) ) {
				define( 'AFP_DOMAIN', 'af-plugin' );
			}
			 ...
			 * Change the second part of the define(), here as 'af-plugin',
			 * to your preferred page slug.
			 *
			 * @since  1.0.0
			 * @return string Returns the URL slug of the admin pages.
			 */
			if ( ! defined( 'AFP_ADMIN_SLUG' ) ) {
				define( 'AFP_ADMIN_SLUG', 'af-plugin' );
			}
			 ...
			 */
			if ( ! defined( 'AFP_DEFAULT_META_IMAGE' ) ) {
				define(
					'AFP_DEFAULT_META_IMAGE',
		...
		private function __clone() {
			...
			_doing_it_wrong( __FUNCTION__, __( 'This is not allowed.', 'af-plugin' ), '1.0.0' );
		...
		private function dependencies() {
			...
			require_once AFP_PATH . 'includes/class-init.php';

			// Include the activation class.
			require_once AFP_PATH . 'includes/class-activate.php';

			// Include the deactivation class.
			require_once AFP_PATH . 'includes/class-deactivate.php';
	 ...
	 * @since  1.0.0
	 * @access public
	 * @return object Returns the instance of the `AF_Plugin` class.
	 */
	function afp_plugin() {

		return AF_Plugin::instance();
...
function afp_about_link( $links ) {
	...
	if ( is_admin() ) {
		...
		if ( afp_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'afp_site_plugin_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'afp_site_plugin_link_position' );
		}

		if ( true == $position ) {
			$url = admin_url( 'index.php?page=' . AFP_ADMIN_SLUG . '-settings' );
		} else {
			$url = admin_url( 'admin.php?page=' . AFP_ADMIN_SLUG . '-settings' );
		}

		// Create new settings link array as a variable.
		$about_page = [
			sprintf(
				'<a href="%1s" class="' . AFP_ADMIN_SLUG . '-page-link">%2s</a>',
				admin_url( 'plugins.php?page=' . AFP_ADMIN_SLUG . '-page' ),
				esc_attr( 'Documentation', 'af-plugin' )
			),
		];
...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'afp_about_link' );
 ...
 * @link   https://codex.wordpress.org/Plugin_API/Filter_Reference/plugin_action_links_(plugin_file_name)
 */
function afp_settings_links( $links, $file ) {
	...
	if ( is_admin() ) {
		...
		if ( afp_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'afp_settings_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'afp_site_settings_position' );
		}
		...
		if ( $file == plugin_basename( __FILE__ ) ) {
			...
			$links[] = sprintf(
				'<a href="%1s" class="' . AFP_ADMIN_SLUG . '-settings-link">%2s</a>',
				$url,
				esc_attr( 'Site Settings', 'af-plugin' )
			);
			$links[] = sprintf(
				'<a href="%1s" class="' . AFP_ADMIN_SLUG . '-scripts-link">%2s</a>',
				admin_url( 'options-general.php?page=' . AFP_ADMIN_SLUG . '-scripts' ),
				esc_attr( 'Script Options', 'af-plugin' )
			);

			// Add a placeholder for an upgrade link.
			$links[] = sprintf(
				'<a href="%1s" title="%2s" class="' . AFP_ADMIN_SLUG . '-upgrade-link" style="color: #888; cursor: default;">%3s</a>',
				''/* Add upgrade URL here */,
				__( 'Upgrade not available', 'af-plugin' ),
				esc_attr( 'Upgrade', 'af-plugin' )
			);
...
add_filter( 'plugin_row_meta', 'afp_settings_links', 10, 2 );
