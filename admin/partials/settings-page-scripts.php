...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'af-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'afp_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'afp-scripts-general';
    $fields  = 'afp-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'afp-scripts-vendor';
    $fields  = 'afp-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'afp_section_script_options', $section );
$do_fields  = apply_filters( 'afp_fields_script_options', $fields );
 ...
 */
if ( 'general' == $active_tab  ) {
    $save = __( 'Save General', 'af-plugin' );
} elseif ( 'vendor' == $active_tab ) {
    $save = __( 'Save Vendor', 'af-plugin' );
} else {
    $save = __( 'Save Settings', 'af-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'afp_save_script_options', $save );

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'af-plugin' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper left.', 'af-plugin' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper right.', 'af-plugin' ); ?></p>
    <?php endif; ?>
