...
class Settings_Page_Site {
    ...
    public function settings_page() {
		...
		if ( afp_acf_options() ) {
			...
			$position   = get_field( 'afp_settings_link_position', 'option' );

			// The label of the page in the admin menu.
			$link_label = get_field( 'afp_site_settings_link_label', 'option' );

			// The icon used for the pagin in the admin menu.
			$link_icon  = get_field( 'afp_site_settings_link_icon', 'option' );
			...
			// Otherwise use "Site Settings" as the label.
				...
				$label = __( 'Site Settings', 'af-plugin' );
			}
			...
			if ( 'top' == $position ) {
				...
				$settings = apply_filters( 'controlled_chaos_site_settings_page_top', [
					'page_title' => $title . __( ' Settings', 'af-plugin' ),
					'menu_title' => $label,
					'menu_slug'  => AFP_ADMIN_SLUG . '-settings',
					'icon_url'   => $icon,
					'position'   => 3,
		...
		// If the Advanced Custom Fields Pro plugin is not active.
			...
			// The label of the page in the admin menu.
			$link_label = sanitize_text_field( get_option( 'afp_site_settings_link_label' ) );

			// The icon used for the pagin in the admin menu.
			$link_icon  = sanitize_text_field( get_option( 'afp_site_settings_link_icon' ) );
			...
			// Otherwise use the Admin Settings icon.
				...
				$icon = __( 'dashicons-admin-settings', 'af-plugin' );
			}
			...
			if ( isset( $_GET['page'] ) ) {
				...
				if (
					...
					&& ( $_GET['page'] == AFP_ADMIN_SLUG . '-settings' )
				) {
					wp_redirect( admin_url( 'admin.php?page=' . AFP_ADMIN_SLUG . '-settings&tab=admin-menu' ), 302 );
				...
				// If on the top level page and top level option is not selected.
					...
					wp_redirect( admin_url( 'index.php?page=' . AFP_ADMIN_SLUG . '-settings&tab=admin-menu' ), 302 );
				}
			}
	...
	public function help_acf_settings_notice() {
		...
		include_once AFP_PATH . 'admin/partials/help/help-acf-notice.php';
    ...
    public function page_help_section_sidebar() {
		...
		$html = '<ul>
			<li><a href="https://www.advancedcustomfields.com/resources/" target="_blank" style="text-decoration: none;">' . __( 'ACF Documentation', 'af-plugin' ) . '</a></li>
		</ul>';
    ...
    public function page_output() {
		...
		require AFP_PATH . 'admin/partials/settings-page-site.php';
	...
	public function acf_docs_link() {
		...
		if ( afp_acf_options() ) {
			...
			$submenu['edit.php?post_type=acf-field-group'][] = [ __( 'Documentation', 'af-plugin' ), 'manage_options', $url ];
 ...
 */
function afp_settings_page_site() {
