...
class Dashboard {
    ...
    public function metaboxes() {
        ...
        if ( afp_acf_options() ) {
            ...
            $hide = get_field( 'afp_dashboard_hide_widgets', 'option' );
        ...
        // If Advanced Custom Fields is not active.
            ...
            $activity   = get_option( 'afp_hide_activity' );
    ...
    public function add_help() {
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Welcome Panel', 'af-plugin' ),
			'content'  => null,
    ...
    public function help_dashboard_sidebar() {
        ...
        $html  = sprintf(
            '<h4>%1s %2s</h4>',
            __( 'Custom Dashboard for', 'af-plugin' ),
             get_bloginfo( 'name' )
        );
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( AFP_ADMIN_SLUG . '-dashboard', AFP_URL .  'admin/dashboard/assets/css/dashboard.min.css', [], null, 'screen' );
        }
