...
class Admin_Pages {
    ...
    public function __construct() {
        ...
        if ( afp_acf_options() ) {
            ...
            $admin_header = get_option( 'afp_use_admin_header' );
        }
    ...
    public function about_plugin() {
        ...
        if ( afp_acf_options() ) {
            ...
            // Get the field.
            $link_label = get_field( 'afp_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress/ClassicPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'afp_site_plugin_link_label' ) );
        }
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'af-plugin' );
        }
        ...
        if ( afp_acf_options() ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'afp_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress/ClassicPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'afp_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'af-plugin' );
        }
    ...
    public function about_plugin_output() {
        ...
        require AFP_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'af-plugin' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'af-plugin' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once AFP_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once AFP_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'af-plugin' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'af-plugin' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            ...
            __( 'for more free downloads.', 'af-plugin' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'af-plugin' )
         );
	...
	public function admin_menus() {
		...
		register_nav_menus( [
			'admin-header' => __( 'Admin Header Menu', 'af-plugin' )
		] );
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once AFP_PATH . 'admin/partials/admin-header.php';
		}
	...
	public function admin_header_layout() {
        ...
        $style .= '.gutenberg-editor-page .afp-admin-header { padding-left: 20px; }';
        $style .= '.gutenberg-editor-page .afp-admin-header:before { content: ""; display: block; width: 100%; height: 56px; }';

        // End the style block.
        $style .= '</style>';

        // Apply a filter for custom admin themeing.
        $style = apply_filters( 'afp_admin_header_layout', $style );
    ...
    public function title_placeholders( $title ) {
        ...
        // Post type: page.
            ...
            $post_title = esc_html__( 'Page Title', 'af-plugin' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'af-plugin' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'afp_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'afp_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'af-plugin' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'afp_image_column_head', $name );
